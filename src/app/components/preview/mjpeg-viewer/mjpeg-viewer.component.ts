...
import { Component, computed, effect, ElementRef, Inject, Injector, OnInit, viewChild } from '@angular/core';
...
export class MjpegViewerComponent implements OnInit {
  ...
  preview_delay = computed(() => Math.floor(this.raspiConfig.config().divider / Math.max(this.raspiConfig.config().video_fps,1) * 1000))
  ...
  checkStatus(){
    this.subscription = timer(100).pipe(
      mergeMap( () => this.raspiConfig.getStatusMjpeg(this.raspiConfig.status_mjpeg()))
    ).subscribe({
      next: (data) => {
  ...
  updatePreview(cycle:boolean){
    ...
    if (cycle !== undefined && cycle == true) {
        ...
        let url = `/cam/cam_pic_new?time=${time}&delay=${this.preview_delay()}`;
        this.mjpeg_src = url
    }
  ...
  loadImg() {
    ...
    if (!this.settingsService.mjpeg_mode()){
      if(!this.raspiConfig.halted() && this.preview_delay()) {
        let url = `/cam/cam_pic?time=${time}&delay=${this.preview_delay()}`
        this.mjpeg_src = url
      } else {
        this.mjpeg_src = "./img/unavailable.png";
      }
  ...
  errorImg() {
    let time = new Date().getTime();
    timer(100).subscribe(
      () => this.mjpeg_src=`/cam/cam_pic?time=${time}`
    )
  }
